...
export async function buildProposalPdf(opts: {
  ...
  sections: Array<
    | {
        kind: "exec";
        title: string;                      // eg "Executive Summary"
        problem?: string;
        solution?: string;
        roi?: string;
        kpis?: KPI[];                       // rendered as small tables
      }
    | {
        kind: "analysis";
        title: string;                      // eg "Detailed Analysis"
        narrative?: string;
        table?: TableDef;
      }
    | {
        kind: "plan";
        title: string;                      // eg "Strategic Action Plan"
  ...
  const docDefinition: any = {
    ...
    info: { title, subject: "Executive Report", creator: "Lovable" },
    header: () => ({
      columns: [
